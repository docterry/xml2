Class XML3 {
	...
	__New(param*){
		...
		temp:=ComObject("MSXML2.DOMDocument")
	...
	Add(XPath,att:="",text:="",dup:=0){
		p:="/"
		add:=(next:=this.SSN("//" XPath))?1:0
		last:=SubStr(XPath,InStr(XPath,"/",0,0)+1)
		if(!next.xml){
			next:=this.SSN("//*")
			for a,b in StrSplit(XPath,"/") {
				p.="/" b
				next:=(x:=this.SSN(p))?x:next.AppendChild(this.XML.CreateElement(b))
			}
		}
		if(dup&&add){
	...
	Find(info*){
		...
		if(info.1.NodeName){
			node:=info.2
			find:=info.3
			return0:=info.4!=""?"SelectNodes":"SelectSingleNode"
			search:=info.4
		}
		else {
			node:=info.1
			find:=info.2
			return0:=info.3!=""?"SelectNodes":"SelectSingleNode"
			search:=info.3
		}
		...
		if(InStr(find,"'"))
			return doc[return0](node "[.=concat('" RegExReplace(find,"'","'," Chr(34) "'" Chr(34) ",'") "')]/.." (search?"/" search:""))
		else
			return doc[return0](node "[.='" find "']/.." (search?"/" search:""))
	}
	Get(XPath,Default){
	...
	Save(x*){
		...
		if(this.XML.SelectSingleNode("*").xml="")
			return MsgBox("Errors happened while trying to save " this.file ". Reverting to old version of the XML")
		filename:=this.file?this.file:x.1.1
		ff:=FileOpen(filename,0)
		text:=ff.Read(ff.length)
		ff.Close()
		if(!this[])
			return MsgBox("Error saving the " this.file " XML.  Please get in touch with maestrith if this happens often")
		if(text!=this[])
